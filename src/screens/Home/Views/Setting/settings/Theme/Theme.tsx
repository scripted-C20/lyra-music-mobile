...
const useActive = (id: string) => {
  ...
  const activeLightThemeId = useSettingValue('theme.lightId')
  const activeDarkThemeId = useSettingValue('theme.darkId')
  const isActive = useMemo(() => {
    const currentThemeId = isAutoTheme
      ? themeState.shouldUseDarkColors
        ? activeDarkThemeId
        : activeLightThemeId
      : activeThemeId
    return currentThemeId == id
  }, [activeDarkThemeId, activeLightThemeId, activeThemeId, id, isAutoTheme])
  return isActive
}
  ...
  return (
    showAll || isActive ? (
      <TouchableOpacity style={{ ...styles.item, width: scaleSizeH(ITEM_HEIGHT) }} activeOpacity={0.5} onPress={() => { setTheme(id) }}>
        ...
        <Text style={{ ...styles.name, fontWeight: isActive ? '700' : '600' }} size={12} color={isActive ? color : theme['c-font']} numberOfLines={1}>{name}</Text>
      </TouchableOpacity>
    ) : null
  )
}
