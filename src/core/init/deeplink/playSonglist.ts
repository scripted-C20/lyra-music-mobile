...
const playSongListDetail = async(source: LX.OnlineSource, link: string, playIndex?: number) => {
  ...
  const rawId = decodeURIComponent(link)
  const normalizedId = await resolveListDetailId(source, rawId) || rawId
  const playListId = `${source}__${normalizedId}`
  let list = (await getListDetail(normalizedId, source, 1)).list
  ...
  list = await getListDetailAll(source, normalizedId)
  if (isPlayingList) {
