import { MAIN_USER_API_SESSION_ID, type InitParams, onScriptAction, sendAction, type ResponseParams, type RequestParams } from '@/utils/nativeModules/userApi'
...
export default async(setting: LX.AppSetting) => {
  ...
  onScriptAction((event) => {
    if (event.sessionId !== MAIN_USER_API_SESSION_ID) return
    // console.log('script actuon: ', event)
    switch (event.action) {
      case 'init': {
        const data = event.data as InitParams
        if ((event as { errorMessage?: string }).errorMessage) data.errorMessage = (event as { errorMessage: string }).errorMessage
        handleStateChange(data)
        break
      }
      case 'response':
        handleUserApiResponse(event.data as ResponseParams)
        break
      case 'request': {
        const data = event.data as RequestParams
        sendScriptRequest(data.requestKey, data.url, data.options)
        break
      }
      case 'cancelRequest':
        cancelRequest(event.data as string, 'request canceled')
        break
      case 'log':
