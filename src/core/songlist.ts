...
interface SongListApi {
  getListId?: (id: string) => Promise<unknown> | unknown
  normalizeListInput?: (id: string) => string
}

const normalizeListInputValue = (api: SongListApi | undefined, id: string) => {
  const input = String(id ?? '').trim()
  if (!input) return ''
  if (!api?.normalizeListInput) return input
  try {
    return api.normalizeListInput(input)
  } catch {
    return input
  }
}

const pickResolvedListId = (value: unknown, fallback: string) => {
  if (typeof value == 'string') {
    const nextValue = value.trim()
    return nextValue || fallback
  }
  if (value && typeof value == 'object' && 'id' in value && typeof value.id == 'string') {
    const nextValue = value.id.trim()
    return nextValue || fallback
  }
  return fallback
}
...
export const resolveListDetailId = async(source: LX.OnlineSource, id: string) => {
  const songListApi = musicSdk[source]?.songList as SongListApi | undefined
  const fallbackId = normalizeListInputValue(songListApi, id)
  if (!fallbackId || !songListApi?.getListId) return fallbackId

  try {
    const resolvedId = await songListApi.getListId(fallbackId)
    return pickResolvedListId(resolvedId, fallbackId)
  } catch {
    return fallbackId
  }
}
