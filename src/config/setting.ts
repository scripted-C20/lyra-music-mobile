...
const applyThemeDefaultMigration = (setting: LX.AppSetting) => {
  const isLegacyGreenTheme =
    setting['theme.id'] == 'green' &&
    setting['theme.lightId'] == 'green' &&
    setting['theme.darkId'] == 'black'
  const isLegacyRedAliasTheme =
    setting['theme.id'] == 'happy_new_year' &&
    setting['theme.lightId'] == 'happy_new_year' &&
    setting['theme.darkId'] == 'black'

  if (isLegacyGreenTheme || isLegacyRedAliasTheme) {
    setting['common.isAutoTheme'] = true
    setting['theme.id'] = 'red'
    setting['theme.lightId'] = 'red'
  }
