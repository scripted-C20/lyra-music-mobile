...
const normalizeFontSize = (size: unknown) => {
  if (typeof size != 'number' || !Number.isFinite(size)) return 1
  if (size < 0.8 || size > 1.3) return 1
  return size
}

export const getFontSize = async() => normalizeFontSize(await getData<number>(fontSizeKey))
...
const normalizeOriginInfo = (origin?: LX.UserApi.UserApiOriginInfo): LX.UserApi.UserApiOriginInfo => {
  const subscribeName = normalizeOriginText(origin?.type === 'subscribe' ? origin.subscribeName : '')
  const subscribeUrl = normalizeOriginText(origin?.type === 'subscribe' ? origin.subscribeUrl : '')
...
export const getUserApiList = async(): Promise<LX.UserApi.UserApiInfo[]> => {
  ...
  for (const info of userApis) {
    ...
    const origin = normalizeOriginInfo(info.origin)
    if (JSON.stringify(origin) !== JSON.stringify(info.origin)) {
      info.origin = origin
      updated = true
    }
  }
...
const normalizeUserApiScriptCompareText = (script: string) => {
  return script.replace(/^\uFEFF/, '').replace(/\r\n?/g, '\n').trim()
}

export const addUserApi = async(params: string | LX.UserApi.ImportUserApiParams): Promise<LX.UserApi.UserApiInfo> => {
  const { script, origin } = normalizeImportParams(params)
  ...
  if (userApis.length) {
    const scriptCompareText = normalizeUserApiScriptCompareText(script)
    const storedScripts = await getDataMultiple(userApis.map(api => `${userApiPrefix}${api.id}`))
    for (const [index, [, storedScript]] of storedScripts.entries()) {
      if (typeof storedScript === 'string' && normalizeUserApiScriptCompareText(storedScript) === scriptCompareText) {
        throw new Error(global.i18n.t('user_api_import_duplicate_script_tip', { name: userApis[index].name }))
      }
    }
  }
