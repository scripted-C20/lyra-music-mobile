...
export default {
  ...
  async resolveDetailUrl(link, retryNum = 0) {
    ...
    if (statusCode > 400) return this.resolveDetailUrl(link, ++retryNum)
  ...
  async getListId(id, retryNum = 0) {
    id = String(id ?? '').trim()
    if (!id) return id
    if (/\/playlist[/?]/.test(id)) {
      const listId = /(?:playlistId|id)=(\d+)/.exec(id)?.[1]
      if (!listId) throw new Error('list detail id parse failed')
      return listId
    }
    if (this.regExps.listDetailLink.test(id)) return id.replace(this.regExps.listDetailLink, '$1')
    if (!(/[?&:/]/.test(id))) return id

    const cachedUrl = this.cachedUrl[id]
    if (cachedUrl) return this.getListId(cachedUrl, retryNum)

    const resolvedUrl = await this.resolveDetailUrl(id, retryNum)
    return this.getListId(resolvedUrl, retryNum + 1)
  },
  ...
  getListDetail(id, page, retryNum = 0) { // 获取歌曲列表内的音乐
    ...
    if (/\/playlist[/?]/.test(id)) {
      ...
      return url ? this.getListDetail(url, page) : this.resolveDetailUrl(id, retryNum).then(url => this.getListDetail(url, page, retryNum + 1))
    }
