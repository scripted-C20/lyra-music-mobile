...
export default {
  ...
  getListId(id) {
    id = String(id ?? '').trim()
    if (!id) return id
    if (id.includes('special/single/')) return id.replace(this.regExps.listDetailLink, '$1')
    if (/^https?:/.test(id)) return id.replace(/^.*?http/, 'http')
    if (id.startsWith('id_')) return id.replace('id_', '')
    return id
  },
